...
#SBATCH --output=slurm_logs/char_tfm_generate_tests_%j.out
#SBATCH --error=slurm_logs/char_tfm_generate_tests_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
generate() {
    ...
    cat $translation.log | grep -P "^H" | sed 's/ //g' | sed 's/▁/ /g' | sort -V |cut -f 3- > $translation 
...
flores_dev='/gpfs/scratch/bsc88/bsc88400/tfm_gender_bias/data-bin-tests/flores_dev_char'
flores_devtest='/gpfs/scratch/bsc88/bsc88400/tfm_gender_bias/data-bin-tests/flores_devtest_char'
must_she='/gpfs/scratch/bsc88/bsc88400/tfm_gender_bias/data-bin-tests/must_she_data_char'
