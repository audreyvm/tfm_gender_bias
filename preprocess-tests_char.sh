...
dict='/gpfs/scratch/bsc88/bsc88400/tfm_gender_bias/data-bin-char/dict.en.txt'
for data_dir in $flores_dev_dir $flores_devtest_dir $must_she_dir
do
    python char_split.py < ${data_dir}/test.ca > ${data_dir}/test.char.ca
    python char_split.py < ${data_dir}/test.en > ${data_dir}/test.char.en
done
...
#Flores dev
dest_dir='/gpfs/scratch/bsc88/bsc88400/tfm_gender_bias/data-bin-tests/flores_dev_char'
rm -r $dest_dir
mkdir -p $dest_dir
dict_dir=$dict
...
#Flores devtest
dest_dir='/gpfs/scratch/bsc88/bsc88400/tfm_gender_bias/data-bin-tests/flores_devtest_char'
rm -r $dest_dir
mkdir -p $dest_dir
dict_dir=$dict
...
#Must_She
dest_dir='/gpfs/scratch/bsc88/bsc88400/tfm_gender_bias/data-bin-tests/must_she_data_char'
rm -r $dest_dir
mkdir -p $dest_dir
dict_dir=$dict
